...
// -----------------------------------------------------------------------------
// A3UE Coalition identity helpers
//
// Deserters are created individually instead of through A3A_fnc_spawnGroup.
// Because of that, their identity must be resolved from their actual unitType.
// If the unit belongs to a coalition faction, use that faction's face/voice
// pools. Otherwise fall back to the normal A3AU faction for this side.
//
// This deliberately does NOT merge voice/face pools: a BAF deserter should
// receive a BAF identity, an AFRF deserter an AFRF identity, etc.
// -----------------------------------------------------------------------------

private _fnc_getIdentityFaction = {
    params ["_unit", "_fallbackFaction"];
...
private _fnc_applyDeserterIdentity = {
    params ["_unit", "_fallbackFaction"];

    private _identityFaction = [
        _unit,
        _fallbackFaction
    ] call _fnc_getIdentityFaction;

    private _faces = _identityFaction getOrDefault ["faces", []];
    private _voices = _identityFaction getOrDefault ["voices", []];

    // Some templates can leave one of these arrays empty. Build the identity
    // HashMap only with values that actually exist so selectRandom never runs
    // against an empty array.
    private _identity = createHashMap;

    if (_faces isEqualType [] && {_faces isNotEqualTo []}) then {
        _identity set ["face", selectRandom _faces];
    };

    if (_voices isEqualType [] && {_voices isNotEqualTo []}) then {
        _identity set ["speaker", selectRandom _voices];
    };

    // Let A3A_fnc_setIdentity handle names and any remaining fallback logic.
    // An empty map is still safe and lets the function generate defaults.
    [_unit, _identity] call A3A_fnc_setIdentity;

    #ifdef DEBUG_MODE_FULL
    private _unitType = _unit getVariable ["unitType", ""];
    Debug_3(
        "[Thorne Coalition Identity] Deserter type=%1 faction=%2 speaker=%3",
        _unitType,
...
for "_i" from 0 to _countX do {
    private _unitRandom = selectRandom _unitTypes;

    // Preserve the original mission's spawn-position logic.
    ...
    // Apply the face/voice pool belonging to the actual coalition faction.
    // Falls back to the normal side faction for non-coalition unit types.
    [_unit, _faction] call _fnc_applyDeserterIdentity;
